...
author: "Youzhi Yu, Trent Buskirk"
date: "`r Sys.Date()`"
vignette: >
...
Regarding the colors for the inside and outside, pass a color vector to `outside_colors` and `inside_colors`:
...
We have been discussing using the package to compare two data sources, but the other way to think about using *ggDoubleHeat* is when comparing mean and median of a single source. Let’s use the following `iris` example to demonstrate it:
