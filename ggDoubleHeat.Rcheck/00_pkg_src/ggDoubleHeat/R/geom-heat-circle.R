...
#' value is 2*sqrt(2) for achieving the equal inner and outer areas.
...
geom_heat_circle <- function(outside,
  ...
  # BEFORE (problematic on ggplot2 4.0.0):
  # if(is.null(outside_name)) {outside_name = rlang::expr({{ outside }})}
  # if(is.null(inside_name)) {inside_name = rlang::expr({{ inside }})}

  # AFTER (coerce to plain strings):
  if (is.null(outside_name)) outside_name <- rlang::as_label(rlang::enexpr(outside))
  if (is.null(inside_name))  inside_name  <- rlang::as_label(rlang::enexpr(inside))

  list(geom_circle_outside(ggplot2::aes(fill = {{ outside }})),

       # BEFORE: ggplot2::scale_fill_gradientn(outside_name, colors = outside_colors, ...),
       ggplot2::scale_fill_gradientn(name = outside_name, colors = outside_colors, ...),

       ggnewscale::new_scale_fill(),

       geom_circle_inside(ggplot2::aes(fill = {{ inside }}, r = r)),

       # BEFORE: ggplot2::scale_fill_gradientn(inside_name, colors = inside_colors, ...))
       ggplot2::scale_fill_gradientn(name = inside_name,  colors = inside_colors,  ...))
