...
geom_heat_tri <- function(lower,
  ...
  if (is.null(lower_name)) lower_name <- rlang::as_label(rlang::enexpr(lower))
  if (is.null(upper_name)) upper_name <- rlang::as_label(rlang::enexpr(upper))

  list(geom_lower_tri(ggplot2::aes(fill = {{ lower }})),

       ggplot2::scale_fill_gradientn(name = lower_name, colors = lower_colors, ...),

       ggnewscale::new_scale_fill(),

       geom_upper_tri(ggplot2::aes(fill = {{ upper }})),

       ggplot2::scale_fill_gradientn(name = upper_name, colors = upper_colors, ...))
